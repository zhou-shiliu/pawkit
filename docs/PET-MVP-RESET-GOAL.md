...
| `working` | 用户活跃、需要低打扰 | `waiting` -> `Idle` |
| `attention` | 用户点击宠物 | `wave` -> `jump` -> `Idle` |
| `success` | 外部任务成功或正向事件 | `jump` -> `review` -> `wave` |
| `failed` | 外部任务失败或负向事件 | `failed` -> `Idle` |
| `sleepy` | 长时间无操作 | `waiting` -> `Idle` |
| `movingRight` | 宠物向右移动 | `run` -> `Idle` |
| `movingLeft` | 宠物向左移动 | `run left` -> `run` -> `Idle` |
...
6. 托盘能显示/隐藏、切换或导入宠物、退出应用。
7. 旧照料系统不会出现在 MVP 默认界面。
8. 新增 Pet MVP 验证门禁通过。
9. README / SPEC / 架构文档 / 测试规范使用同一套目标定义。
